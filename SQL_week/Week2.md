...
✔️ 잘못된 부분
: 본 쿼리는 포켓몬을 타입별로 그룹화한 뒤, 그 중 평균 공격력이 60 이상인 타입만을 추출하려는 목적을 가지고 있음.
하지만 WHERE 절은 테이블에서 모든 데이터에 즉시 조건을 적용하기에, 그룹화 이후에 계산되는 '평균값'을 조회하는데 적절하지 않음.
따라서 집계 함수를 조건으로 사용할 때는 데이터를 먼저 '타입'으로 그룹화한 후, HAVING 절을 이용해야 적절한 결과를 얻음.
~~~
