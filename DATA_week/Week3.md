...
```
3번
: 결과 데이터프레임인에는 서로 다른 컬럼명인 df1의 'col1'과 df2의 'col3'를 기준으로 데이터가 매칭되어 있습니다. 따라서 각 프레임의 기준 컬럼을 명시해주는 left_on과 right_on 옵션을 이용해야합니다.
또한, 두 데이터프레임에 공통으로 존재하는 'x', 'y'와 한쪽에만 존재하는 'z'와 'w' 까지 모두 결과에 포함되어 있고, 짝이 없는 부분은 NaN으로 처리되어있기에, 합집합 방식인 how='outer'가 적용되었습니다. 
```
